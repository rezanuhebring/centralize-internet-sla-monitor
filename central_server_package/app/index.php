...
<head>
    ...
    <style>
        ...
        .selector-group select, .time-period-selector button, .chart-zoom-controls button { padding: 8px 12px; border-radius: 4px; border: 1px solid var(--border-color); font-size: 1em; background-color: var(--card-bg); color: var(--text-color); cursor: pointer; }
            ...
            <div id="summary-view" class="hidden">
                ...
                <div class="chart-card card" style="margin-top:20px;">
                    <div class="chart-header">
                        ...
                        <div class="chart-zoom-controls" data-chart="cumulativeRttChart">
                            <button data-zoom="in">Zoom In</button><button data-zoom="out">Zoom Out</button><button data-zoom="reset">Reset</button>
                        </div>
                    </div>
            ...
            <div id="detail-view" class="hidden">
                ...
                <div class="chart-card card" style="margin-top:20px;">
                    <div class="chart-header">
                        ...
                        <div class="chart-zoom-controls" data-chart="rttChart">
                             <button data-zoom="in">Zoom In</button><button data-zoom="out">Zoom Out</button><button data-zoom="reset">Reset</button>
                        </div>
                    </div>
                    ...
                    <div class="chart-container" style="margin-top:20px;"><canvas id="wifiChart"></canvas></div>
                </div>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderDetailView = (data) => {
        ...
        // New Wi-Fi card population
        setText('wifi-ssid', latest?.wifi_ssid);
        setText('wifi-signal', latest?.wifi_signal_strength_percent, '%');
        setText('wifi-band-channel', (latest?.wifi_band && latest?.wifi_channel) ? `${latest.wifi_band} / Ch. ${latest.wifi_channel}` : 'N/A');
        ...
        renderChart('wifiChart', { 'Wi-Fi Signal Strength (%)': 'wifi_signal_strength_percent' }, data.wifi_chart_data, 'timestamp', { yAxisLabel: '%' });
        summaryView.classList.add('hidden'); detailView.classList.remove('hidden');
    };
    
    // UPDATED: More robust chart rendering function
    const renderChart = (canvasId, seriesConfig, data, xKey, options = {}) => {
        ...
        const colors = ['#36a2eb', '#ff6384', '#ff9f40', '#4bc0c0', '#9966ff', '#ffc107'];
        ...
        const datasets = Object.entries(seriesConfig).map(([label, key], index) => {
            ...
            // MRTG Style: Use a less intense color for the fill area
            const borderColor = colors[index % colors.length];
            const backgroundColor = borderColor + '4D'; // ~30% opacity
            ...
            return {
                ...
                tension: 0.3,
                fill: !isLoss, // Fill area for lines, not for bars
                yAxisID: yAxisID,
        ...
        const chartOptions = {
            ...
            scales: {
                ...
                y: {
                    ...
                    grid: { color: gridColor }, ticks: { color: textColor, callback: (value) => `${value}${options.yAxisLabel || ' ms'}` }
                }
            },
            plugins: {
    ...
    const handleZoom = (chartId, direction) => {
    ...
    document.body.addEventListener('click', e => { 
        ...
        if (e.target.closest('.chart-zoom-controls button')) {
            const button = e.target.closest('button');
            const chartId = button.parentElement.dataset.chart;
            const zoomType = button.dataset.zoom;
            handleZoom(chartId, zoomType);
        }
    });
